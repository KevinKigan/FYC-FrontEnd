...
import {regexEscape} from 'ngx-bootstrap/chronos/parse/regex';
...
export class ModeloEspecificoAdminComponent implements OnInit {
  ...
  selects: string[] = ['Marca', 'Modelo', 'Carrocería', 'Transmisión', 'Eje Motriz', 'Turbo', 'Compresor', 'Tipo Combustible', 'Normativa Euro'];
  ...
  private configurarMotores() {
    this.cochesService.getTiposCombustibles().subscribe(combustibles => {
      let tipoCombustibles: TipoCombustible[] = combustibles.tipos_combustibles;
      // combustibles[0].forEach(combustible => tipoCombustibles.push(combustible));
      tipoCombustibles.filter(combustible => {
    ...
    this.cochesService.getNormativasConsumos().subscribe(normativas => {
      let tipoEmisiones: TipoEmisiones[] = normativas.normativas;
      tipoEmisiones.forEach(normativa => {
  ...
  private configurarColumnas(model: any[], index: number) {
    this.arrayRow[index] = [];
    for (let i = 0; i<model.length/this.arrayCol.length;i++){this.arrayRow[index][i]=i}
  }
}
